...
void buildBluetoothConnectionTesting() {
  ...
  if(central) {
    ...
    while(central.connected()) {
		  sendData(_accelerator_x, _accelerator_y, _accelerator_z, _depth, _duration, _gyroscope_x, _gyroscope_y, _gyroscope_z, _heart_freq, _heart_var, _luminance, _oxygen_saturation, _ref_dive, _water_temp);
		  delay(200);
    }
  }
